...
// Generic type for chat IDs
type ChatId = Id<"chats"> | Id<"agent2Chats">;

interface SidebarProps {
  // API endpoints
  listChatsApi: FunctionReference<
    "query",
    "public",
    Record<string, never>,
    any
  >;
  createChatApi: FunctionReference<
    "mutation",
    "public",
    { title: string },
    any
  >;
  deleteChatApi: FunctionReference<"mutation", "public", { id: any }, any>;
  storeMessageApi: FunctionReference<"mutation", "public", any, any>;
  updateChatTitleApi?: FunctionReference<"mutation", "public", any, any>; // Optional

  // UI customization
  title?: string;
  icon?: React.ReactNode;
  basePath: string; // e.g., "/dashboard/chat" or "/dashboard/agent2"
  agentName?: string;
}
...
export default function Sidebar({
  ...
  const chats = useQuery(listChatsApi);
  const createChat = useMutation(createChatApi);
  const deleteChat = useMutation(deleteChatApi);

  // Create new chat
  const handleNewChat = async (chatTitle: string = "New Chat") => {
    try {
      const chatId = await createChat({ title: chatTitle });
  ...
  const handleDeleteChat = async (id: ChatId) => {
    try {
      ...
      // Redirect if currently viewing deleted chat
      if (window.location.pathname.includes(id)) {
        router.push("/dashboard");
      }
    } catch (error) {
